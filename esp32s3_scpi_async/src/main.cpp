...
static uint8_t mcpReadyMask() {
  uint8_t m = 0;
  for (int i = 0; i < MCP23017_COUNT; i++) if (mcpReady[i]) m |= (1 << i);
  return m;
}
...
static const char *paramAt(const ParamList &pl, int index) {
  return (index >= 0 && index < pl.count) ? pl.v[index] : nullptr;
}
...
static void cmdNetStat(Stream &io) {
  ...
  ipToText(cfgIp, ip, sizeof(ip));
  ipToText(cfgGw, gw, sizeof(gw));
  ipToText(cfgMask, mask, sizeof(mask));
  ipToText(cfgDns, dns, sizeof(dns));
  ipToText(currentIp, local, sizeof(local));
...
static void cmdHelp(Stream &io) {
  ...
  io.println(F("ETH:IP?"));
  io.println(F("NET:STAT?"));
  io.println(F("NET:DHCP 0|1"));
  io.println(F("NET:PORT 1..65535"));
  io.println(F("NET:IP a,b,c,d"));
  io.println(F("NET:GW a,b,c,d"));
  io.println(F("NET:MASK a,b,c,d"));
  io.println(F("NET:DNS a,b,c,d"));
  io.println(F("NET:SAVE"));
  io.println(F("NET:RESTART"));
...
static void handleMcpMode(char *params, Stream &io) {
  ...
  char res[OLED_CONSOLE_COLS];
  snprintf(res, sizeof(res), "OK M%d.%d MODE", r.chip, r.localPin);
  oledConsolePush("<", res);
  io.println(F("OK"));
}
...
static void handleMcpWrite(char *params, Stream &io) {
  ...
  char res[OLED_CONSOLE_COLS];
  snprintf(res, sizeof(res), "OK M%d.%d=%d", r.chip, r.localPin, value ? 1 : 0);
  oledConsolePush("<", res);
  io.println(F("OK"));
}
...
static void handleMcpRead(char *params, Stream &io) {
  ...
  char res[OLED_CONSOLE_COLS];
  snprintf(res, sizeof(res), "M%d.%d=%d", r.chip, r.localPin, v);
  oledConsolePush("<", res);
  io.println(v ? F("1") : F("0"));
}
...
static void handleMcpToggle(char *params, Stream &io) {
  ...
  char res[OLED_CONSOLE_COLS];
  snprintf(res, sizeof(res), "OK M%d.%d=%d", r.chip, r.localPin, nv);
  oledConsolePush("<", res);
  io.println(F("OK"));
}
...
static void handleNetIpSet(char *params, Stream &io, IPAddress &target, const char *label) {
  ...
  char ip[18], res[OLED_CONSOLE_COLS];
  ipToText(target, ip, sizeof(ip));
  snprintf(res, sizeof(res), "%s %s", label, ip);
  oledConsolePush("<", res);
  io.println(F("OK"));
}
...
static void handleNetDhcp(char *params, Stream &io) {
  ...
  cfgUseDhcp = v ? true : false;
  saveNetPrefs();
  netRestartRequest = true;
  oledConsolePush("<", cfgUseDhcp ? "DHCP ON" : "DHCP OFF");
  io.println(F("OK"));
}
...
static void handleNetPort(char *params, Stream &io) {
  ParamList pl; parseParams(params, pl);
  int p;
  if (pl.count < 1 || !parseIntStrict(paramAt(pl, 0), p) || p < 1 || p > 65535) {
    replyErr(io, "NET:PORT needs 1..65535");
    return;
  }
  cfgPort = (uint16_t)p;
  saveNetPrefs();
  netRestartRequest = true;
  char res[OLED_CONSOLE_COLS];
  snprintf(res, sizeof(res), "PORT %u", cfgPort);
  oledConsolePush("<", res);
  io.println(F("OK"));
}
...
static void handleOneCommand(char *cmdLine, Stream &io) {
  ...
  } else if (!strcmp(line, "MCP:STAT?")) {
    cmdMcpStat(io);
    oledConsolePush("<", "MCP STAT OK");
  } else if (!strcmp(line, "NET:STAT?")) {
    cmdNetStat(io);
    oledConsolePush("<", "NET STAT OK");
  } else if (!strcmp(line, "NET:DHCP")) {
    handleNetDhcp(params, io);
  } else if (!strcmp(line, "NET:PORT")) {
    handleNetPort(params, io);
  } else if (!strcmp(line, "NET:IP")) {
    handleNetIpSet(params, io, cfgIp, "IP");
  } else if (!strcmp(line, "NET:GW")) {
    handleNetIpSet(params, io, cfgGw, "GW");
  } else if (!strcmp(line, "NET:MASK")) {
    handleNetIpSet(params, io, cfgMask, "MASK");
  } else if (!strcmp(line, "NET:DNS")) {
    handleNetIpSet(params, io, cfgDns, "DNS");
  } else if (!strcmp(line, "NET:SAVE")) {
    saveNetPrefs();
    oledConsolePush("<", "NET SAVED");
    io.println(F("OK"));
  } else if (!strcmp(line, "NET:RESTART")) {
    netRestartRequest = true;
    oledConsolePush("<", "NET RESTART");
    io.println(F("OK"));
...
static void initI2c() {
  ...
  if (oledReady) {
    ...
    oled.println(F("encoder IP menu"));
    oled.display();
  }
  xSemaphoreGive(i2cMutex);
}
...
static void initW5500() {
  ...
  char txt[OLED_CONSOLE_COLS];
  char ip[18];
  ipToText(currentIp, ip, sizeof(ip));
  snprintf(txt, sizeof(txt), "NET %s:%u", ip, cfgPort);
  oledConsolePush("<", txt);
}
...
static void handleEncoderButton() {
  ...
  if (uiMode == UI_MENU) {
    ...
    if (menuIndex == 0) uiMode = UI_EDIT_IP;
      ...
      netRestartRequest = true;
      uiMode = UI_CONSOLE;
      oledConsolePush("<", "SAVE+NET RESTART");
    } else {
      uiMode = UI_CONSOLE;
    }
    return;
  ...
  if (uiMode == UI_EDIT_DHCP || uiMode == UI_EDIT_PORT) {
    saveNetPrefs();
    netRestartRequest = true;
    uiMode = UI_MENU;
    return;
...
static void drawMenu() {
  ...
  if (first < 0) first = 0;
  if (first > MENU_ITEMS - 5) first = MENU_ITEMS - 5;
  for (int i = first; i < first + 5 && i < MENU_ITEMS; i++) {
...
static void drawEditIp(const char *title, const IPAddress &ip) {
  ...
  oled.println(title);
  for (int i = 0; i < 4; i++) {
    if (i == editOctet) oled.print('[');
    oled.print(ip[i]);
    if (i == editOctet) oled.print(']');
    if (i < 3) oled.print('.');
  }
  oled.println();
  oled.println(F("Turn=change"));
  oled.println(F("Press=next/save"));
  oled.display();
}
...
static void drawEditDhcp() {
  ...
  oled.setTextColor(SSD1306_WHITE);
  oled.setCursor(0, 0);
  oled.println(F("DHCP mode"));
  oled.println(cfgUseDhcp ? F("ON") : F("OFF"));
  oled.println(F("Turn=toggle"));
  oled.println(F("Press=save"));
  oled.display();
}
...
static void drawEditPort() {
  ...
  oled.setTextColor(SSD1306_WHITE);
  oled.setCursor(0, 0);
  oled.println(F("TCP SCPI Port"));
  oled.println(cfgPort);
  oled.println(F("Turn=change"));
  oled.println(F("Press=save"));
  oled.display();
}
...
static void acceptClient() {
  ...
  for (int i = 0; i < MAX_ETH_CLIENTS; i++) {
    if (!ethClients[i] || !ethClients[i].connected()) {
      ...
      ethClients[i].println(F("4x MCP23017, encoder IP/PORT menu"));
      ethClients[i].println(F("Send HELP? NET:STAT? MCP:STAT?"));
      return;
    }
  }
}
...
void taskEthernet(void *) {
  while (true) {
    if (netRestartRequest) {
      netRestartRequest = false;
      initW5500();
    }
  ...
  while (true) {
    if (oledReady) {
      ...
      if (uiMode == UI_MENU) drawMenu();
        ...
        oled.print(F(" P:")); oled.println(cfgPort);
...
void taskEncoder(void *) {
  ...
  while (true) {
    ...
    if (lastSw == HIGH && sw == LOW && millis() - lastButtonMs > ENC_BUTTON_DEBOUNCE_MS) {
      lastButtonMs = millis();
      handleEncoderButton();
    }
    lastSw = sw;
...
void setup() {
  ...
  Serial.println(F("USB Serial and TCP enabled"));
  Serial.println(F("Send HELP? NET:STAT? MCP:STAT?"));
